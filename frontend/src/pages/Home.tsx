...
import { useOverviewStats } from "@/lib/overviewStats";
import { buildCategoryModel, type CategoryChartModel } from "@/lib/categoryChart";
import { createSampleState } from "@/lib/sampleData";

/* Everything below is fetched on demand: the first mobile paint only pays for the
   overview shell, not recharts, the other tabs, or the PDF builder. */
const named = <T extends string>(loader: () => Promise<Record<string, unknown>>, key: T) =>
  lazy(() => loader().then((mod) => ({ default: mod[key] as React.ComponentType<never> })));

const TransactionsPanel = named(() => import("@/components/TransactionsPanel"), "TransactionsPanel");
const BudgetGuardrails = named(() => import("@/components/BudgetGuardrails"), "BudgetGuardrails");
const AccountsPanel = named(() => import("@/components/AccountsPanel"), "AccountsPanel");
const InsightsPanel = named(() => import("@/components/InsightsPanel"), "InsightsPanel");
const GoalsPanel = named(() => import("@/components/GoalsPanel"), "GoalsPanel");
const CommitmentsPanel = named(() => import("@/components/CommitmentsPanel"), "CommitmentsPanel");
const WishlistManager = named(() => import("@/components/WishlistManager"), "WishlistManager");
const SettingsPanel = named(() => import("@/components/SettingsPanel"), "SettingsPanel");
const PDFReportModal = named(() => import("@/components/PDFReportModal"), "PDFReportModal");
const MobileOverview = named(() => import("@/components/mobile/MobileOverview"), "MobileOverview");
const CashFlowArea = named(() => import("@/components/charts/OverviewCharts"), "CashFlowArea");
const CategoryStackedBars = named(() => import("@/components/charts/OverviewCharts"), "CategoryStackedBars");
const CategoryDonut = named(() => import("@/components/charts/OverviewCharts"), "CategoryDonut");

function PanelFallback({ height = 220 }: { height?: number }) {
  return (
    <div className="grid w-full animate-pulse place-items-center rounded-2xl bg-secondary/30" style={{ minHeight: height }} data-testid="panel-fallback">
...
export default function Home() {
  ...
  // Active categories + anything already used by history, so nothing ever disappears.
  ...
  const currentTransactions = useMemo(() => state.transactions.filter((item) => monthKey(item.date) === compareMonth), [state.transactions, compareMonth]);
  const previousTransactions = useMemo(() => state.transactions.filter((item) => monthKey(item.date) === previous), [state.transactions, previous]);
  ...
  // Top 5 categories + an automatic "Lainnya" bucket, shared by every chart.
  const categoryModel = useMemo(
    () => buildCategoryModel(currentTransactions, previousTransactions, displayRate, state.locale),
    [currentTransactions, previousTransactions, displayRate, state.locale],
  );
  ...
  const handleAddTransaction = (event: React.FormEvent) => {
    ...
    setTransactionForm({ kind: "expense", amount: "", description: "", category: categories[0] ?? "Other", accountId: state.accounts[0]?.id ?? "", currency: state.baseCurrency, date: new Date().toISOString().slice(0, 10), tags: "" });
  ...
  const openAddTransaction = () => { setEditingTransaction(null); setTransactionForm((form) => ({ ...form, accountId: form.accountId || state.accounts[0]?.id || "", category: categories.includes(form.category) ? form.category : (categories[0] ?? "Other") })); setShowTransactionForm(true); };
  ...
  return (
    <div className="min-h-svh bg-background text-foreground">
      <div className="mx-auto flex min-h-svh max-w-[1600px]">
        ...
        <main className="min-w-0 flex-1 pb-24 lg:pb-8">
          ...
          <div className="px-4 py-5 sm:px-6 sm:py-8 lg:px-10">
            <Suspense fallback={<PanelFallback height={360} />}>
              {tab === "overview" && (isMobile
                ? <MobileOverview state={state} t={t} totalBalance={totalBalance} currentSpend={currentSpend} currentIncome={currentIncome} categoryModel={categoryModel} flowChart={flowChart} currentMonth={compareMonth} setCompareMonth={setCompareMonth} onNavigate={setTab} onLoadSample={loadSample} accountName={accountName} />
                : <Overview state={state} t={t} totalBalance={totalBalance} currentSpend={currentSpend} currentIncome={currentIncome} committed={committed} trendText={trendText} previousSpend={previousSpend} categoryModel={categoryModel} flowChart={flowChart} currentMonth={compareMonth} setCompareMonth={setCompareMonth} onAdd={() => openAddTransaction()} onNavigate={setTab} onLoadSample={loadSample} accountName={accountName} />)}
              {tab === "transactions" && <TransactionsPanel state={state} labels={{ all: t.all, type: t.type, expense: t.expense, incomeType: t.incomeType, category: t.category, account: t.account, newest: t.newest, largest: t.largest, search: t.search, noData: t.noData, addTransaction: t.addTransaction }} categories={categories} filteredTransactions={filteredTransactions} filter={filter} setFilter={setFilter} accountName={(accountId: string) => state.accounts.find((account) => account.id === accountId)?.name ?? "—"} onAdd={() => openAddTransaction()} onEdit={openEditTransaction} onDelete={deleteTransaction} />}
      ...
      {showPdfModal && <Suspense fallback={null}><PDFReportModal state={state} onClose={() => setShowPdfModal(false)} /></Suspense>}
    </div>
  );
}
...
function Overview({
  ...
  categoryModel: CategoryChartModel;
  ...
  const recent = useMemo(() => [...state.transactions].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 5), [state.transactions]);
      ...
      <div className="mb-6 grid gap-4 xl:grid-cols-[1.35fr_1fr]">
        ...
        <Card className="border-border/70 bg-card/75 p-6 backdrop-blur-xl">
          ...
          <div className="mt-5 h-[105px] w-full">
            <Suspense fallback={<PanelFallback height={105} />}>
              <CashFlowArea data={flowChart} format={money} gradientId="cashflow" />
            </Suspense>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.45fr_1fr]">
        <Card className="border-border/70 bg-card/75 p-5 sm:p-6">
          ...
          <div className="h-[280px] w-full">
            <Suspense fallback={<PanelFallback height={280} />}>
              <CategoryStackedBars
                model={categoryModel}
                format={money}
                compactAxis={(value) => formatMoney(value, state.baseCurrency, state.locale, true)}
              />
            </Suspense>
          </div>
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5 text-[10px] font-semibold text-muted-foreground" data-testid="category-chart-legend">
            {categoryModel.keys.map((key) => (
              <span key={key} className="flex items-center gap-1">
                <i className="size-2 rounded-full" style={{ backgroundColor: categoryModel.colorFor(key) }} />
                {key}
              </span>
            ))}
            {categoryModel.keys.length === 0 && <span>{t.noData}</span>}
          </div>
          {categoryModel.collapsedCount > 0 && (
            <p className="mt-2 text-[10px] text-muted-foreground" data-testid="category-chart-others-note">
              {isId
                ? `"Lainnya" menggabungkan ${categoryModel.collapsedCount} kategori: ${categoryModel.collapsedNames.slice(0, 6).join(", ")}${categoryModel.collapsedCount > 6 ? ", …" : ""}`
                : `"Others" groups ${categoryModel.collapsedCount} categories: ${categoryModel.collapsedNames.slice(0, 6).join(", ")}${categoryModel.collapsedCount > 6 ? ", …" : ""}`}
            </p>
          )}
        </Card>
        ...
        <Card className="border-border/70 bg-card/75 p-5 sm:p-6">
          ...
          <div className="flex items-center gap-4">
            <div className="h-[150px] w-[150px] shrink-0">
              <Suspense fallback={<PanelFallback height={150} />}>
                <CategoryDonut model={categoryModel} format={money} emptyLabel={t.noData} />
              </Suspense>
            </div>
            <div className="min-w-0 flex-1 space-y-3" data-testid="allocation-legend">
              {categoryModel.slices.map((item) => (
                <div key={item.category} className="flex items-center gap-2">
                  <span className="size-2 shrink-0 rounded-full" style={{ backgroundColor: categoryModel.colorFor(item.category) }} />
              ...
              {categoryModel.slices.length === 0 && <p className="text-xs text-muted-foreground">{t.noData}</p>}
            </div>
          </div>
        </Card>
      </div>
