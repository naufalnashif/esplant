...
import type { GmailCandidate } from "@/lib/integrationsApi";
...
export default function Home() {
  ...
  const stateQuery = useQuery({ queryKey: ["nusa-artha-local-state"], queryFn: loadState, initialData: createInitialState(), staleTime: Infinity });
  ...
  const importMockEmails = (items: GmailCandidate[]) => {
    const fresh = items.filter((item) => !state.transactions.some((transaction) => transaction.id === item.external_id)).map((item) => ({ id: item.external_id, kind: "expense" as TransactionKind, date: item.date, description: item.merchant, category: item.category, accountId: state.accounts.find((account) => account.brand.toLowerCase() === item.account_hint?.toLowerCase())?.id ?? state.accounts[0]?.id ?? "", amount: item.amount, currency: (item.currency as Currency) ?? "IDR", baseAmount: item.amount * (state.exchangeRates[(item.currency as Currency) ?? "IDR"] ?? 1), tags: ["qris", "gmail"] }));
    ...
    <div className="min-h-svh bg-background text-foreground">
      <div className="mx-auto flex min-h-svh max-w-[1600px]">
        <aside className="sticky top-0 hidden h-svh w-[250px] shrink-0 flex-col border-r border-border/70 bg-card/70 px-5 py-6 backdrop-blur-xl lg:flex" data-testid="desktop-sidebar">
          <div className="mb-10 flex items-center gap-3 px-2"><div className="grid size-10 place-items-center rounded-2xl bg-primary text-primary-foreground shadow-lg shadow-primary/20"><CircleDollarSign size={22} /></div><div><p className="font-heading text-lg font-extrabold tracking-tight">Esplant</p><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">Money, made clear</p></div></div>
        ...
        <main className="min-w-0 flex-1 pb-24 lg:pb-8">
          ...
          <div className="px-4 py-6 sm:px-6 sm:py-8 lg:px-10">
            ...
            {tab === "settings" && <SettingsPanel state={state} profileDraft={profileDraft || state.profileName} setProfileDraft={setProfileDraft} updateState={updateState} onSaveProfile={() => { updateState({ profileName: profileDraft || state.profileName }); toast.success("Profil lokal tersimpan."); }} onCsv={exportCsv} onJson={exportJson} onImport={importJson} onErase={eraseAll} onPrint={() => window.print()} onReminder={requestReminder} />}
          </div>
          ...
          {tab === "settings" && <div className="px-4 pb-8 sm:px-6 lg:px-10"><DataHealthPanel state={state} onSave={save} /></div>}
          {tab === "settings" && <div className="px-4 pb-8 sm:px-6 lg:px-10"><IntegrationSetup /></div>}
          {tab === "settings" && <div className="px-4 pb-8 sm:px-6 lg:px-10"><GmailImporter state={state} onImport={importMockEmails} /><ReportCenter state={state} currentMonth={compareMonth} onPrint={() => window.print()} /></div>}
          {tab === "settings" && <div className="px-4 pb-8 sm:px-6 lg:px-10"><ReportSendButton state={state} /></div>}
          {tab === "settings" && <div className="px-4 pb-8 sm:px-6 lg:px-10"><CategoryManager state={state} onSave={(categories) => updateState({ categories })} /></div>}
      ...
      {showTransactionForm && <TransactionModal state={state} t={t} categories={categories} form={transactionForm} setForm={setTransactionForm} onChange={updateTransaction} onClose={() => { setShowTransactionForm(false); setEditingTransaction(null); }} onSubmit={handleAddTransaction} />}
    </div>
  );
}
...
function Overview({ state, t, totalBalance, currentSpend, currentIncome, committed, trendText, previousSpend, categoryChart, flowChart, currentMonth, setCompareMonth, onAdd, onNavigate, accountName }: { state: FinanceState; t: typeof copy.id; totalBalance: number; currentSpend: number; currentIncome: number; committed: number; trendText: string; previousSpend: number; categoryChart: { category: string; current: number; previous: number }[]; flowChart: { month: string; income: number; expense: number }[]; currentMonth: string; setCompareMonth: (value: string) => void; onAdd: () => void; onNavigate: (tab: Tab) => void; accountName: (id: string) => string }) {
  ...
  return <div className="animate-rise-in"><SectionHeading eyebrow="Personal finance / 01" title={state.profileName ? `${t.hello}, ${state.profileName}.` : `${t.hello}.`} description={t.command} action={<Button data-testid="add-transaction-button" onClick={onAdd} className="gap-2 shadow-lg shadow-primary/20"><Plus size={17} />{t.addTransaction}</Button>} />
    {state.accounts.length === 0 && <Card className="mb-6 border-primary/25 bg-primary/5 p-6 sm:p-8" data-testid="onboarding-banner"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-4"><div className="grid size-12 shrink-0 place-items-center rounded-xl bg-primary/15 text-primary"><WalletCards size={22} /></div><div><h2 className="font-heading text-lg font-bold">{state.locale === "id" ? "Mulai dari nol — data Anda 100% di perangkat ini" : "Start from zero — your data stays 100% on this device"}</h2><p className="mt-1 max-w-xl text-sm leading-relaxed text-muted-foreground">{state.locale === "id" ? "Tambahkan akun pertama (bank, e-wallet, cash) untuk mulai mencatat, atau impor backup JSON dari perangkat lain." : "Add your first account (bank, e-wallet, cash) to start tracking, or import a JSON backup from another device."}</p></div></div><div className="flex shrink-0 flex-wrap gap-2"><Button data-testid="onboarding-add-account-button" onClick={() => onNavigate("accounts")} className="gap-2"><Plus size={15} />{state.locale === "id" ? "Tambah akun" : "Add account"}</Button><Button data-testid="onboarding-import-button" variant="outline" onClick={() => onNavigate("settings")} className="gap-2">{state.locale === "id" ? "Impor JSON" : "Import JSON"}</Button></div></div></Card>}
    <div className="mb-6 grid gap-4 xl:grid-cols-[1.35fr_1fr]"><Card className="relative min-h-[218px] overflow-hidden border-primary/20 bg-gradient-to-br from-[#303030] via-[#262626] to-[#1c1c1c] p-6 text-white shadow-xl shadow-primary/10 sm:p-8"><div className="absolute -right-20 -top-24 size-72 rounded-full border-[30px] border-white/8" /><div className="absolute -bottom-28 right-24 size-56 rounded-full border-[18px] border-white/6" /><div className="relative flex h-full flex-col justify-between"><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/65">{t.totalBalance} · {state.baseCurrency}</p><p className="mt-3 font-heading text-4xl font-extrabold tracking-tight sm:text-5xl" data-testid="total-balance-value">{formatMoney(totalBalance, state.baseCurrency, state.locale)}</p></div><div className="rounded-xl border border-white/15 bg-white/10 p-2"><WalletCards size={20} /></div></div><div className="mt-9 flex flex-wrap items-center gap-5 text-xs text-white/70"><span className="flex items-center gap-1.5"><span className="size-2 rounded-full bg-emerald-300" /> {state.accounts.length} active accounts</span><span className="flex items-center gap-1.5"><ShieldCheck size={14} /> Encrypted on device</span></div></div></Card><Card className="border-border/70 bg-card/75 p-6 backdrop-blur-xl"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Smart snapshot</p><p className="mt-2 font-heading text-lg font-bold">{t.cashFlow}</p></div><Badge variant="secondary" className="gap-1"><RefreshCw size={12} /> Live</Badge></div><div className="mt-5 h-[105px] w-full"><ResponsiveContainer width="100%" height="100%"><AreaChart data={flowChart}><defs><linearGradient id="cashflow" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ffa116" stopOpacity={0.42} /><stop offset="100%" stopColor="#ffa116" stopOpacity={0} /></linearGradient></defs><Tooltip contentStyle={{ background: "#282828", border: "1px solid #3c3c3c", borderRadius: 12, fontSize: 11 }} formatter={(value) => formatMoney(Number(value), state.baseCurrency, state.locale, true)} /><Area type="monotone" dataKey="income" stroke="#2cbb5d" strokeWidth={2} fill="url(#cashflow)" /><Area type="monotone" dataKey="expense" stroke="#ef4743" strokeWidth={2} fill="transparent" /></AreaChart></ResponsiveContainer></div><div className="mt-3 flex gap-4 text-[10px] font-semibold text-muted-foreground"><span className="flex items-center gap-1"><i className="size-2 rounded-full bg-emerald-400" />Income</span><span className="flex items-center gap-1"><i className="size-2 rounded-full bg-red-400" />Expense</span></div></Card></div>
