...
export function InsightsPanel({
  ...
  const allInsights = categories
    .map((category) => {
      ...
      const shape = (severity: number, tone: string, icon: typeof ShieldAlert, title: string, detail: string) =>
        ({ category, severity, current, tone, icon, title, detail });

      if (budget && ratio >= 1)
        return shape(4, "rose", ShieldAlert, `${category} melebihi budget`, "Kurangi pengeluaran agar kembali ke batas.");
      if (budget && ratio >= 0.8)
        return shape(3, "amber", Lightbulb, `${category} hampir penuh`, `${Math.max(0, Math.round((1 - ratio) * 100))}% ruang budget tersisa bulan ini.`);
      if (change >= 15)
        return shape(2, "amber", ArrowUpRight, `${category} naik ${change}%`, "Bandingkan transaksi sebelum menambah pengeluaran lagi.");
      if (change <= -15)
        return shape(1, "teal", ArrowDownRight, `${category} turun ${Math.abs(change)}%`, "Berhasil menghemat dibanding bulan lalu.");
      return shape(0, "indigo", Sparkles, `${category} terkendali`, budget ? "Masih dalam batas budget." : "Budget aman.");
    })
    // Only categories that actually moved keep a card; the rest would be noise.
    .filter((insight) => insight.current > 0 || insight.severity > 0)
    .sort((a, b) => b.severity - a.severity || b.current - a.current);

  // Cap the grid at 4 cards so many categories stay readable; the tail is summarised.
  const categoryInsights = allInsights.slice(0, 4);
  const hiddenInsights = allInsights.length - categoryInsights.length;
      ...
      <Card className="border-border/70 bg-card/75 p-5 shadow-sm backdrop-blur-xl sm:p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          {categoryInsights.length === 0 && (
            <p className="col-span-full py-8 text-center text-xs text-muted-foreground" data-testid="insights-empty-state">
              {isId ? "Belum ada pengeluaran bulan ini untuk dianalisis." : "No spending to analyse this month yet."}
            </p>
          )}
        </div>
        {hiddenInsights > 0 && (
          <p className="mt-3 text-[11px] font-semibold text-muted-foreground" data-testid="insights-hidden-count">
            {isId
              ? `+${hiddenInsights} kategori lain digabung sebagai "Lainnya" agar tetap mudah dibaca.`
              : `+${hiddenInsights} more categories grouped as "Others" to keep this readable.`}
          </p>
        )}
      </Card>
    </div>
