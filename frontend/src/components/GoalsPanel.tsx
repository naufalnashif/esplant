...
export function GoalsPanel({
  ...
  >;
  onAddWish: (event: React.FormEvent) => void;
  onCommit: (
    goal: SavingsGoal,
    amount: number,
  ...
  const [goalId, setGoalId] = useState(state.savings[0]?.id ?? "");
  const [amount, setAmount] = useState("");
  const [direction, setDirection] = useState<"deposit" | "withdraw">("deposit");
  const displayed =
    category === "all"
      ? state.wishlist
      : state.wishlist.filter((item) => item.category === category);
  const savedTotal =
    state.savings.reduce(
      (sum, goal) => sum + goal.saved * state.exchangeRates[goal.currency],
      0,
    ) / (state.exchangeRates[state.baseCurrency] || 1);
  const submitCommit = (event: React.FormEvent) => {
    ...
    <div className="animate-rise-in">
      <div className="mb-6">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
          Tabungan adalah bucket uang yang nyata; wishlist adalah rencana yang
          terpisah.
        </p>
      </div>
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-primary/20 bg-primary/8 p-5">
          <p className="text-xs font-semibold text-muted-foreground">
            Total uang tertabung
          </p>
          <p
            className="mt-2 font-heading text-2xl font-extrabold"
            data-testid="total-savings-value"
          >
            {money(savedTotal, state.baseCurrency, state.locale)}
          </p>
          <p className="mt-1 text-[10px] text-primary">
            Across {state.savings.length} goals
          </p>
        </div>
        <div className="rounded-2xl border border-border/70 bg-card/75 p-5">
          ...
          <p className="mt-2 font-heading text-2xl font-extrabold">
            {money(
              state.savings.reduce((sum, goal) => sum + goal.target, 0),
              state.baseCurrency,
              state.locale,
            )}
          </p>
        ...
        <div className="rounded-2xl border border-border/70 bg-card/75 p-5">
          ...
          <p className="mt-1 text-[10px] text-muted-foreground">
            Filtered by category below
          </p>
        </div>
      </div>
      <div className="grid gap-6 xl:grid-cols-[1.1fr_1fr]">
        <div className="space-y-6">
          <section className="rounded-2xl border border-border/70 bg-card/75 p-5 shadow-sm backdrop-blur-xl sm:p-6">
              ...
              {state.savings.map((goal) => (
                ...
                >
                  ...
                  <div className="mt-2 flex justify-between text-[10px] font-semibold text-muted-foreground">
                    <span>
                      {money(goal.saved, goal.currency, state.locale)}
                    </span>
                    <span>
                      {money(goal.target, goal.currency, state.locale)}
                    </span>
                  </div>
                </div>
              ))}
          ...
          <section className="rounded-2xl border border-primary/20 bg-primary/5 p-5 sm:p-6">
            <div className="mb-4 flex items-center gap-2">
              <ArrowUpFromLine size={17} className="text-primary" />
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                className={`flex items-center gap-1 rounded-lg px-3 py-2 text-[10px] font-bold ${direction === "deposit" ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground"}`}
              >
                <ArrowUpFromLine size={12} /> Menabung
              </button>
              <button
                type="button"
                data-testid="savings-withdraw-toggle"
                onClick={() => setDirection("withdraw")}
                className={`flex items-center gap-1 rounded-lg px-3 py-2 text-[10px] font-bold ${direction === "withdraw" ? "bg-amber-400 text-slate-900" : "border border-border text-muted-foreground"}`}
              >
                <ArrowDownToLine size={12} /> Ambil tabungan
              </button>
            </div>
            <p className="mt-3 text-[10px] leading-relaxed text-muted-foreground">
              Commit mengurangi saldo akun dan membuat transaksi Savings.
              Withdraw melakukan kebalikannya.
            </p>
          </section>
        </div>
        <div className="space-y-6">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      {wish.status} ·{" "}
                      {money(wish.price, wish.currency, state.locale)}
                    </p>
                  </div>
                </div>
              ))}
          ...
          <section id="wishlist-form-section" className="rounded-2xl border border-border/70 bg-card/75 p-5 shadow-sm backdrop-blur-xl sm:p-6">
            <div className="mb-4 flex items-center gap-2">
              <Plus size={17} className="text-primary" />
              <h2 className="font-heading text-lg font-bold">{editingWishId ? "Edit wishlist" : "Add wishlist"}</h2>
            </div>
            <form onSubmit={onAddWish} className="grid gap-3 sm:grid-cols-2">
              <input
                ...
                value={wishForm.name}
                onChange={(event) =>
                  setWishForm((form) => ({ ...form, name: event.target.value }))
                }
              ...
              <div className="flex gap-2 sm:col-span-2">
                {editingWishId && setEditingWishId && (
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => {
                      setEditingWishId(null);
                      setWishForm({ name: "", price: "", priority: "medium", targetDate: new Date().toISOString().slice(0, 10), category: wishlistCategories[0] });
                    }}
                    className="h-10 px-4"
                  >
                    Cancel
                ...
                >
                  {editingWishId ? "Save changes" : "Save wishlist"}
                </Button>
              </div>
            </form>
          </section>
