...
import { MAX_CATEGORIES } from "@/lib/categoryChart";
...
export function CategoryManager({
  ...
  const add = () => {
    ...
    if (!name) return;
    if (name.length > 24) {
      toast.error(isId ? "Nama kategori maksimal 24 karakter." : "Category name is limited to 24 characters.");
      return;
    }
    ...
    if (state.categories.length >= MAX_CATEGORIES) {
      toast.error(
        isId
          ? `Maksimal ${MAX_CATEGORIES} kategori. Arsipkan atau hapus kategori lain dulu agar chart tetap mudah dibaca.`
          : `Limit is ${MAX_CATEGORIES} categories. Archive or delete one first to keep charts readable.`,
      );
      return;
    }
    ...
    <section className="rounded-2xl border border-border/70 bg-card/75 p-4 shadow-sm backdrop-blur-xl sm:p-5" data-testid="category-manager">
      ...
      <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-1.5 self-start sm:self-auto">
          <Badge variant="secondary" className="text-xs" data-testid="category-count-badge">
      ...
      {state.categories.length >= MAX_CATEGORIES && (
        <p className="mb-3 rounded-xl border border-amber-500/35 bg-amber-500/8 px-3 py-2 text-[11px] font-semibold text-amber-500" data-testid="category-limit-notice">
          {isId
            ? `Batas ${MAX_CATEGORIES} kategori tercapai. Chart hanya menampilkan 5 kategori terbesar, sisanya digabung jadi "Lainnya".`
            : `Limit of ${MAX_CATEGORIES} categories reached. Charts show the top 5 and group the rest as "Others".`}
        </p>
      )}
