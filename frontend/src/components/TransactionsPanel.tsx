...
export function TransactionsPanel({ state, labels, categories, filteredTransactions, filter, setFilter, accountName, onAdd, onEdit, onDelete }: { state: FinanceState; labels: TransactionLabels; categories: string[]; filteredTransactions: Transaction[]; filter: FilterState; setFilter: React.Dispatch<React.SetStateAction<FilterState>>; accountName: (accountId: string) => string; onAdd: () => void; onEdit: (transaction: Transaction) => void; onDelete: (transaction: Transaction) => void }) {
  ...
  return <div className="animate-rise-in">
    ...
    <div className="space-y-2 md:hidden" data-testid="transactions-mobile-list">
      ...
      {hiddenCount > 0 && <button type="button" data-testid="transactions-show-all-button" onClick={() => setShowAll(true)} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-primary/35 text-xs font-bold text-primary">{isId ? `Lihat semua transaksi (${filteredTransactions.length})` : `Show all transactions (${filteredTransactions.length})`}</button>}
      {showAll && filteredTransactions.length > MOBILE_PREVIEW && <button type="button" data-testid="transactions-show-less-button" onClick={() => setShowAll(false)} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl border border-border/70 text-xs font-bold text-muted-foreground">{isId ? "Tampilkan lebih sedikit" : "Show less"}</button>}
      {filteredTransactions.length === 0 && <div className="rounded-2xl border border-border/70 bg-card/75 px-5 py-14 text-center text-sm text-muted-foreground">{labels.noData}</div>}
    </div>

    {/* Desktop/tablet: full table */}
    <div className="hidden overflow-hidden rounded-2xl border border-border/70 bg-card/75 md:block"><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left"><thead className="border-b border-border/70 bg-secondary/35 text-[10px] uppercase tracking-widest text-muted-foreground"><tr><th className="px-5 py-4">Date</th><th className="px-5 py-4">Description</th><th className="px-5 py-4">Category</th><th className="px-5 py-4">Account</th><th className="px-5 py-4 text-right">Amount</th><th className="px-5 py-4 text-right">Manage</th></tr></thead><tbody className="divide-y divide-border/50">{filteredTransactions.map((item) => <tr key={item.id} data-testid={`transaction-row-${item.id}`} className="group hover:bg-secondary/30"><td className="px-5 py-4 text-xs text-muted-foreground">{shortDate(item.date, state.locale)}</td><td className="px-5 py-4"><div className="flex items-center gap-3"><div className={`grid size-8 place-items-center rounded-lg ${item.kind === "income" ? "bg-emerald-500/12 text-emerald-400" : "bg-red-500/12 text-red-400"}`}>{item.kind === "income" ? <ArrowDownLeft size={14} /> : <ArrowUpRight size={14} />}</div><div><p className="text-sm font-semibold">{item.description}</p><p className="text-[10px] text-muted-foreground">{item.tags.join(" · ") || "untagged"}</p></div></div></td><td className="px-5 py-4"><Badge variant="outline">{item.category}</Badge></td><td className="px-5 py-4 text-xs text-muted-foreground">{accountName(item.accountId)}</td><td className={`px-5 py-4 text-right font-data text-xs font-bold ${item.kind === "income" ? "text-emerald-400" : ""}`}>{item.kind === "income" ? "+" : "−"}{formatMoney(item.baseAmount, state)}</td><td className="px-5 py-4"><div className="flex justify-end gap-1"><button type="button" data-testid={`transaction-edit-${item.id}-button`} onClick={() => onEdit(item)} className="grid size-8 place-items-center rounded-lg text-muted-foreground hover:bg-primary/10 hover:text-primary"><Pencil size={13} /></button><button type="button" data-testid={`transaction-delete-${item.id}-button`} onClick={() => onDelete(item)} className="grid size-8 place-items-center rounded-lg text-muted-foreground hover:bg-red-400/10 hover:text-red-400"><Trash2 size={13} /></button></div></td></tr>)}{filteredTransactions.length === 0 && <tr><td colSpan={6} className="px-5 py-14 text-center text-sm text-muted-foreground">{labels.noData}</td></tr>}</tbody></table></div></div></div>;
}
